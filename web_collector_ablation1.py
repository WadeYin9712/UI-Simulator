...
@timer
def webarena_sim_traj_ablation1(init_state: dict, 
                      domain: str,
                      min_step=3, 
                      max_step=5, 
                      index=1, 
    ...
    if save:
        ...
        # print("############")
        if rag_enabled:
            directory = f'train_set_{domain}_ablation1_rag/init_state_{index}_maxlen={max_step}/iter_{num}'
        else:
            directory = f'train_set_{domain}_ablation1/init_state_{index}_maxlen={max_step}/iter_{num}'
                ...
                for i, post_reasoning_task in enumerate(post_reasoning_tasks):
                    ...
                    if 'None' not in answer:
                        ...
                        if save:
                            ...
                            if rag_enabled:
                                directory = f'train_set_{domain}_ablation1_rag/init_state_{index}_maxlen={max_step}/iter_{num}_reasoning_{i}'
                            else:
                                directory = f'train_set_{domain}_ablation1/init_state_{index}_maxlen={max_step}/iter_{num}_reasoning_{i}'
                            os.makedirs(directory, exist_ok=True)
...
############################################
def process_sim_collect_ablation1(init_state, domain, num, min_step=3, max_step=5, index=1, save=True, rag_enabled=False):
    # print(guides)
...
def collect_ablation1(domain, init_states, num_guides_per_step, min_length_range: list, nums: list, debug=False, rag_enabled=True):
    ...
    if debug:
        ...
        for i in range(nums[0]):
            webarena_sim_traj_ablation1(init_states[0], domain, guide, None, num_guides_per_step, min_step=min_step, max_step=min_step+6, index=1000, num=i, save=True, general_entity=.0, rag_enabled=rag_enabled)
...
if __name__ == '__main__':
    ...
    collect_ablation1(args.domain, init_states, args.min_steps, args.nums, debug=args.debug, rag_enabled=args.rag_enabled)
