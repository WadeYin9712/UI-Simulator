...
from android_world.agents import vllm_t3a
from android_world.agents import vllm_agentgrow
from android_world.agents import explore_agentgrow
...
_MODEL_NAME = flags.DEFINE_string(
    ...
    'The model name to use for the agent. This is only used for agents that'
    ' require a model name, such as T3A and M3A.',
)
...
_REMOVE_METADATA = flags.DEFINE_boolean(
    'remove_metadata',
    False,
    'Whether to remove metadata from the observations.'
)

_DO_SHUFFLE = flags.DEFINE_boolean(
    'do_shuffle',
    False,
    'Whether to shuffle the elements in the observations.'
)
...
def _get_agent(
  ...
  # SeeAct.
    ...
    agent = vllm_t3a.T3AVllm(env, _MODEL_NAME.value)
  elif _AGENT_NAME.value == 'agentgrow_vllm':
    agent = vllm_agentgrow.AgentgrowVllm(env, model_name=_MODEL_NAME.value, remove_metadata=_REMOVE_METADATA.value, do_shuffle=_DO_SHUFFLE.value)
  elif _AGENT_NAME.value == 'agentgrow_explore':
    agent = explore_agentgrow.AgentgrowExplore(env)
